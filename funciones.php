...
function anadirPedido($conexion){
    ...
    for ($i=0; $i < count($pizzas); $i++) { 
        $array[$i]=$pizzas[$i]. "_". $ingredientes[$i];
    }

    $contar=array_count_values($array);

    foreach($contar as $pizza=>$count) { 

        $unidades=$count;
        $split = explode("_", $pizza);

        if($split[1] == "Ninguno"){
            $sql="INSERT INTO LineaPedido values (id_lineaPedido,'$num_pedido','" . $split[0] . "',$unidades,null,0)";
        }else{
            $sql="INSERT INTO LineaPedido values (id_lineaPedido,'$num_pedido','" . $split[0] . "',$unidades,'" . $split[1] . "',$unidades)";
        } 
...
function borrarPizzas($conexion){
    ...
    if (isset($_REQUEST['Borrar'])){
        ...
        if($_REQUEST['borrarSelect'] == "0"){
            ...
            echo "<fieldset><h2>Error: Esta pizza no se puede borrar. Elimine los ingredientes de la pizza primero. <br> Disculpe las molestias.<br><br>
              <a href='borrarPizzas.php'>Volver</a></h2></fieldset>";
        }else{
            mysqli_query($conexion,$sql);
...
function borrarIngredientes($conexion){
	if (isset($_REQUEST['Borrar'])){
          ...
          if($_REQUEST['borrarSelect'] == "0"){
            ...
            echo "<fieldset><h2>Error: Este ingrediente no se puede borrar. Elimine el ingrediente de todas las pizzas primero. <br> Disculpe las molestias.<br><br>
              <a href='borrarIngredientes.php'>Volver</a></h2></fieldset>";
          }else{
            mysqli_query($conexion,$sql);
